...
const getAIMove = (board, difficulty) => {
    ...
    const MAX_DEPTH = (difficulty === 'easy') ? 1 : 2; // Beginner: 1-ply search, Intermediate: 2-ply search
...
const GameContainer = ({ gameMode = 'self', aiDifficulty = null, onGameOver }) => {
    ...
    const performMove = useCallback((fromId, toId, currentBoard, currentCaptured) => {
        ...
        // 3. Check Win Condition (King Captured)
        ...
        <div className="flex flex-col items-center gap-6 p-4 md:flex-row md:items-start md:justify-center md:gap-10 w-full max-w-6xl mx-auto">
            ...
            <div className="order-1 md:order-2">
                <ChessBoard 
                    ...
                    // Flip board for black player in self-play mode (optional aesthetic)
                    flipped={gameMode === 'self' && turn === 'black'} 
                />
            </div>
        </div>
    );
...
const AISelectView = ({ setPage, setAiDifficulty }) => {
    const difficulties = [
        ...
        { level: 'Intermediate', desc: '1-Ply MiniMax (Ready for 2-Ply Expansion)', color: 'bg-yellow-500', difficulty: 'medium' },
        { level: 'Advanced', desc: 'Currently 1-Ply (Ready for deeper search)', color: 'bg-red-500', difficulty: 'hard' },
    ];
